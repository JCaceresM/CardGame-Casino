...
class Table {
  ...
  deleteCard(cardToDelete, playerInTurn) {
    this.deck.cardsForTable.forEach((card) => {
      const index = this.deck.cardsForTable.findIndex(
        (card) => card.img === cardToDelete
      );
      if (index != -1) {
        playerInTurn.lotOfcard.push(this.deck.cardsForTable[index]);
  ...
  makePosibleCombinations() {
    ...
    const combiningTheCardOnTheTable = (card) => {
      for (let index = c; index < this.deck.cardsForTable.length; index++) {
        let combinatedCards = [];
        if (
          card.img != this.deck.cardsForTable[index].img &&
          card.value + this.deck.cardsForTable[index].value < 15
        ) {
          combinatedCards = [
...
class Game {
  ...
  takeCombination(e) {
    ...
    if (index != -1) {
      this.searchOthersToMacth(
        this.players[this.activePlayer].cardsPlayer[index].value
      );
      this.handleCombinationProcess(
        combinatedCards4,
        this.players[this.activePlayer].cardsPlayer[index]
      );
    } else {
  ...
  scoring() {
    ...
    this.verifyWinner =
      Math.max(...this.players.map((player) => player.score)) > 20
        ? true
        : false;
  }
  ...
  showWinnerMessage(players) {
    ...
    h2.innerText =
      "El ganador es:" + players[index].name + " Id: " + players[index].id;
